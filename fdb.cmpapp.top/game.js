...
Events.on(render, 'afterRender', function() {
    ...
    if (gameState.inclineAngleDegrees) {
        ...
        if (gameState.incline && gameState.currentObject) {
            ...
            // Top edge - now invisible
            ctx.beginPath();
            ctx.moveTo(topLeftX, topLeftY);
            ctx.lineTo(topRightX, topRightY);
            ctx.strokeStyle = 'rgba(255, 255, 255, 0)'; // Completely transparent
            ctx.lineWidth = 0; // Zero width
            ctx.stroke();
            
            // Left edge - now invisible
            ctx.beginPath();
            ctx.moveTo(topLeftX, topLeftY);
            ...
            // Draw point at 1/3 distance on top edge - now invisible
            const t = 1/3;
            const pointX = topLeftX + t * (topRightX - topLeftX);
            const pointY = topLeftY + t * (topRightY - topLeftY);
            
            ctx.beginPath();
            ctx.arc(pointX, pointY, 0, 0, 2 * Math.PI); // Zero radius
            ctx.fillStyle = 'rgba(255, 255, 255, 0)'; // Completely transparent
            ctx.fill();
            
            // Draw all four corners of the inclined plane - now invisible
            const cornerSize = 0; // Zero size
            
            // Draw corners - all invisible
            ctx.fillStyle = 'rgba(255, 255, 255, 0)'; // Completely transparent
            ...
            ctx.beginPath();
            ctx.arc(bottomLeftX, bottomLeftY, cornerSize, 0, 2 * Math.PI);
            ctx.fillStyle = 'rgba(255, 255, 255, 0)'; // Completely transparent
            ctx.fill();
            ctx.strokeStyle = 'rgba(255, 255, 255, 0)'; // Completely transparent
            ctx.lineWidth = 0; // Zero width
            ...
            // Draw horizontal plane (ground) with top and bottom edges - now invisible
            const groundHeight = 20; // Same as inclineHeight for consistency
            const groundTopY = groundY - groundHeight/2;
            const groundBottomY = groundY + groundHeight/2;
            
            // Draw top edge of horizontal plane - now invisible
            ctx.beginPath();
            ...
            // Draw bottom edge of horizontal plane - now invisible
            ctx.beginPath();
            ...
            // Fill the horizontal plane with transparent color - now invisible
            ctx.fillStyle = 'rgba(255, 255, 255, 0)'; // Completely transparent
            ctx.fillRect(0, groundTopY, groundWidth, groundHeight);
            
            // Draw horizontal line from bottom-left corner - now invisible
            ctx.beginPath();
            ctx.moveTo(bottomLeftX, bottomLeftY);
...
function drawAngleIndicatorInCanvas(ctx, position, angleDegrees) {
    const x = position.x;
    const y = position.y;
    const radius = 60; // Larger radius for the angle indicator
    
    // Save context state
    ctx.save();
    
    // Draw inclined reference line (representing bottom edge of inclined plane)
    // Making it invisible by matching background color
    ...
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x - radius, y); // Extended leftwards
    ctx.strokeStyle = 'rgba(255, 255, 255, 0)'; // Completely transparent
    ctx.lineWidth = 0; // Zero width
    ctx.stroke();
    
    // Draw the inner angle directly ABOVE the horizontal plane
    // Making it visible but subtle
    const innerAngleDeg = angleDegrees;
    
    // Draw the arc for the inner angle using a wedge
    ctx.beginPath();
    
    // Start at the center point
    ctx.moveTo(x, y);
    
    // Draw the first line (along the horizontal reference) - extended leftwards
    ctx.lineTo(x - radius * 0.75, y); // Using 75% of the radius for the wedge
    
    // Draw the arc for the inner angle - going COUNTERCLOCKWISE from horizontal to inclined
    ctx.arc(x, y, radius * 0.75, Math.PI, Math.PI + (innerAngleDeg * Math.PI / 180), false);
    
    // Close the path back to the center
    ctx.closePath();
    
    // Fill with a subtle but visible color
    ctx.fillStyle = 'rgba(255, 255, 0, 0.15)'; // Light yellow, subtle but visible
    ctx.fill();
    
    // Draw the outline
    ctx.strokeStyle = 'rgba(0, 0, 0, 0.2)'; // Subtle dark outline
    ctx.lineWidth = 1;
    ctx.stroke();
    
    // Get the positions of the horizontal and inclined plane labels
    const groundWidth = physicsWorld.clientWidth;
    const inclineMidX = position.x - 100; // Approximate position based on the intersection point
    
    // Calculate the midpoint between the horizontal and inclined plane labels horizontally
    const midpointX = (groundWidth / 2 + inclineMidX) / 2;
    
    // Keep the original vertical position
    const labelY = position.y - 15; // This was the original vertical offset
    
    // Use the new horizontal position and original vertical position
    const labelX = midpointX;
    
    ctx.font = 'bold 14px Arial';
    ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'; // Dark text for good visibility
